...
พอลองจะเปิดให้ทีมหรือเพื่อนใช้โมเดลร่วมกัน ปัญหาก็เริ่มชัด: SSH port forwarding มันเหมาะกับ "คนเดียวชั่วคราว" ไม่ใช่ "หลายคนถาวร"

ครั้งนี้ผมจะเล่าวิธีที่สะอาดกว่ามาก — `tailscale serve` — คำสั่งเดียวที่เอา service บน localhost มาเสิร์ฟเป็น HTTPS จริงๆ บนโดเมนของตัวเอง พร้อมระบบ "รู้ว่าใครกำลังใช้" ที่ใช้งานได้ทันทีโดยไม่ต้องเขียน auth system แม้แต่บรรทัดเดียว
...
## 01: ปัญหาที่แท้จริง — localhost เป็นเกาะ

TL;DR ก่อน สำหรับคนที่เข้าใจ infrastructure อยู่แล้ว:

> ถ้าอยากเปิด local LLM server ให้ใช้จากไหนก็ได้ใน tailnet แบบมี HTTPS และรู้ว่าใครเป็นคนยิง request — รัน `tailscale serve localhost:8080` บนเครื่อง server แล้วจบ ไม่ต้องตั้งค่าอะไรอีกเลย
...
- GPU 2 ใบ (16GB ต่อใบ)
- รัน `llama-server` โหลด Qwen3 27B ตัว quant Q4_K_M (ไฟล์ ~16GB) — offload ลง GPU ทั้งหมด
- Server นี้ listen อยู่ที่ `http://127.0.0.1:8080` — คือ localhost เท่านั้น

อยากให้ทุก device ใน tailnet เปิด web UI / endpoint นี้ได้จากที่ไหนก็ได้ โดยไม่ต้องเปิด terminal ค้างและไม่ต้องตั้ง reverse proxy เอง

คำสั่งเดียว บนเครื่อง server:
...
จากตรงนี้ ทุก device ที่ login อยู่ใน tailnet เปิด `https://phon-server.my-tailnet.ts.net` ได้เลย — TLS certificate จัดให้อัตโนมัติ ไม่มี warning browser และถ้า device หลับ/สลับเครือข่ายแล้วกลับมา มันก็ใช้ได้ทันที เพราะ traffic ถูก route ผ่าน Tailscale daemon ที่รันเป็น service อยู่แล้ว ไม่ผูกกับ SSH session ของใคร

ห้าบรรทัดล้มค่า SSH tunnel ที่ผมใช้มาตลอดหลายเดือนคับ

## 02: มันทำงานยังไง — สามชั้นที่ต้องแยกให้ออก

คนที่สับสนกับ Tailscale ส่วนใหญ่ สับสนเพราะมันมี "ชั้น" สามชั้นที่ทำงานคนละหน้าที่ แต่ชื่อคล้ายกันมาก

**ชั้นที่ 1: Tailnet (พื้นฐาน)** — เครือข่าย WireGuard ที่ Tailscale สร้างให้ ทุก device ที่ login ด้วย account เดียวกัน (หรือถูกเชิญ) เจอกันหมดผ่าน IP ภายใน 100.x.x.x — นี่คือพื้นที่ที่ทุกอย่างเกิดขึ้น ถ้าไม่มีชั้นนี้ก็ไม่มีอะไรต่อจากนี้

**ชั้นที่ 2: Tailscale Serve** — reverse proxy ที่รันอยู่ใน daemon ของเครื่อง server มันรับ HTTPS เข้าที่ `https://ชื่อเครื่อง.tailnet.ts.net` แล้วส่งต่อ (proxy) ไปที่ localhost ของเครื่องตัวเอง — เห็นได้จากบรรทัด `|-- / proxy http://127.0.0.1:8080` ใน output ด้านบน

**ชั้นที่ 3: Tailscale Funnel** — ตัวขยายช่องทางจาก "คนใน tailnet" เป็น "คนบนอินเทอร์เน็ตทั้งเผ่าพันธุ์" — ใช้เมื่อต้องการให้คนที่ไม่มี Tailscale เข้าถึงได้

จำง่ายๆ:

- **tailnet** = อาคาร (เครือข่าย)
- **serve** = เปิดประตูห้องให้คนในอาคารเข้าได้
- **funnel** = เปิดประตูอาคารให้คนนอกเข้าได้

สองชั้นบนคือหัวใจของบทความนี้ — เดี๋ยวเจาะทีละตัว
...
และข้อจำกัดที่คนพลาดบ่อย: **port เดียวกันรันทั้ง Serve และ Funnel พร้อมกันไม่ได้** — ถ้าคำสั่งล่าสุดที่รันคือ `serve` port นั้นจะ private, ถ้าคำสั่งล่าสุดคือ `funnel` port นั้นจะ public ทันที ตัว command หลัง wins เสมอ
...
จุดที่ผมว่าน่าสนใจที่สุด: **relay server decrypt ข้อมูลไม่ได้** — เพราะ TLS connection เป็นแบบ end-to-end ระหว่าง browser ของคนเรียกกับเครื่องคุณ Tailscale แค่หล่อ traffic ผ่าน ซึ่งหมายความว่าแม้แต่ Tailscale เองก็อ่านข้อมูลที่วิ่งผ่าน funnel ไม่ได้
...
นี่คือ feature ที่ผมคิดว่าถูกออกแบบมาเพื่อ use case แบบ "shared LLM server" พอดีทั้งๆ ที่ Tailscale ไม่เคยโฆษณามันแบบนั้น
...
และเพื่อกัน spoof: Tailscale จะ **strip headers เหล่านี้ออกจาก incoming request เสมอ** — ถ้ามีใครพยายามส่ง `Tailscale-User-Login: billgates@microsoft.com` มาเอง header นั้นจะถูกลบทิ้งก่อนถึง backend แล้วเติมของจริงลงไป ทำให้ backend ไว้ใจค่าพวกนี้ได้เต็มที่ (ถ้ามาทางนี้)

ตัวอย่างที่ใช้ได้จริงกับ llama-server หรือ backend อื่น เช่น FastAPI:
...
> **ข้อควรระวังที่ docs เน้นตรงๆ**: backend ต้อง listen บน `127.0.0.1` เท่านั้น — ถ้าเปิดให้ LAN หรือ tailnet เข้าถึง port 8080 โดยตรง (ไม่ผ่าน serve) ใครก็ตั้ง header เหล่านี้เองได้ง่ายๆ และ backend จะเชื่อ เพราะฉะนั้น localhost-only เสมอ
...
และชุด subcommands ใหม่สำหรับจัดการ config แบบ declarative:

- `tailscale serve status` — ดูว่า serve อะไรอยู่
- `tailscale serve reset` — ล้าง config ทั้งหมด
- `tailscale serve get-config` / `set-config` — อ่าน/ตั้งค่าแบบ structured

**สิ่งที่ต้องเปิดก่อนใช้**: tailnet ต้องเปิด HTTPS certificates ไว้ — ถ้ายังไม่เปิด CLI จะพาไปหน้า web consent ให้กดอนุญาตครั้งเดียวแล้วจบ (wizard ทำให้หมด) และ ACL rules ของ tailnet ก็ยังใช้กับ traffic ของ Serve — ถ้า ACL block device ตัวไหน device นั้นเข้าไม่ได้แม้จะเห็น URL ก็ตาม
...
สำหรับ use case ของผม (homelab ส่วนตัว + แชร์ให้เพื่อนไม่กี่คน): แผน Personal ฟรีเกินความจำเป็น — 6 users กับ devices ไม่จำกัด ครอบคลุมทุกอย่างที่ผมต้องการ และ Serve/Funnel ใช้ได้ทุกแผนไม่มีการ lock ไว้

สังเกตว่าราคาใช้โครงสร้าง seat-based — จ่ายตามจำนวน user ที่ "นั่ง" ใน tailnet ไม่ใช่ตามจำนวน devices (devices ของ user ไม่จำกัดทุกแผน)

## 09: Pro Tips แยกตามระดับ

**มือใหม่ (เพิ่งเริ่ม):**

- รัน `tailscale serve localhost:8080` แล้วเปิด URL จากมือถือที่ลง Tailscale ไว้ — ทดสอบว่าทุกอย่างทำงานก่อนเลย ใช้เวลาไม่กี่นาที
- ใช้ flag `--http=80` เพื่อเข้าผ่าน `http://phon-server` (MagicDNS) ได้เลยไม่ต้องพิมพ์ชื่อเต็ม
...
- อ่าน `Tailscale-User-Login` จาก backend เพื่อทำ per-user logging/quota — ฟรีและไม่ต้องเขียน auth เอง
- ใช้ `tailscale serve status` ตรวจสอบว่า serve อะไรอยู่บ้างหลังผ่านไปสักพัก (config สะสมได้)
...
**การแชร์ service ไม่ใช่ปัญหา networking — มันปัญหาการจัดการพอร์ต ให้ระบบที่เรียบร้อยแล้วช่วยจัดการแทน**

สิ่งที่ `tailscale serve` ทำคือดึงงาน "ตั้ง reverse proxy + TLS + auth" ที่เรามักเก็บไว้ทำ "วันหลัง" ออกไปจากสมการ ทำให้เหลือแค่คำสั่งเดียวที่รันแล้วได้ผลจริง

สำหรับคนที่รัน local LLM อยู่แล้วแบบผม นี่คือชิ้นที่หายไป: โมเดลรันบนเครื่องที่บ้าน แต่ประตูสู่โมเดลยังเปิดไม่ถูก — `tailscale serve` คือประตูบานนั้น ที่ติดตั้งเสร็จในคำสั่งเดียวและบอกได้ด้วยว่าใครกำลังเดินเข้ามา
